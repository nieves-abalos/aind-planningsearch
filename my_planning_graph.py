...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # for example, the A0 level will iterate through all possible actions for the problem and add a PgNode_a
        # to a_levels[0] set if all prerequisite literals for the action hold in S0.  This can be accomplished by testing
        #   to see if a proposed PgNode_a has prenodes that are a subset of the previous S level.  Once an
        #   action node is added, it MUST be connected to the S node instances in the appropriate s_level set.
        # print("\n>>>> New invoke: add_action_level")
    ...
    def add_literal_level(self, level):
        ...
        # print("\n>>>> New invoke: add_literal_level")
        self.s_levels.append(set())
        for action in self.a_levels[level-1]:
            literals = action.effnodes
            for literal in literals:
                literal.children.add(action)
                action.parents.add(literal)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # print("\n>>>> New invoke: inconsistent_effects_mutex")
        #print(node_a1.show())
        #print(node_a2.show())

        for effectAdd in node_a1.action.effect_add:
            for effectRem in node_a2.action.effect_rem:
                if effectAdd == effectRem:
                    return True

        for effectAdd in node_a2.action.effect_add:
            for effectRem in node_a1.action.effect_rem:
                if effectAdd == effectRem:
                    return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # print("\n>>>> New invoke: interference_mutex")
        #print(node_a1.show())
        #print(node_a2.show())

        for effectAdd in node_a1.action.effect_add:
            for precRem in node_a2.action.precond_neg:
                if effectAdd == precRem:
                    return True

        for effectAdd in node_a2.action.effect_add:
            for precRem in node_a1.action.precond_neg:
                if effectAdd == precRem:
                    return True

        for effectRem in node_a1.action.effect_rem:
            for precAdd in node_a2.action.precond_pos:
                if effectRem == precAdd:
                    return True

        for effectRem in node_a2.action.effect_rem:
            for precAdd in node_a1.action.precond_pos:
                if effectRem == precAdd:
                    return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # print("\n>>>> New invoke: competing_needs_mutex")
        # TODO test for Competing Needs between nodes
        
        if self.inconsistent_support_mutex(node_a1, node_a2):
            return True
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        # print("\n>>>> New invoke: negation_mutex (between literals)")
        # TODO test for negation between nodes
        #print(node_s1.show())
        #print(node_s2.show())

        if (node_s1.symbol == node_s2.symbol) and (node_s1.is_pos != node_s2.is_pos):
            #print("TRUE (node_s1.symbol == node_s2.symbol) and (node_s1.is_pos != node_s2.is_pos)")
            return True

        #print("FALSE (node_s1.symbol == node_s2.symbol) and (node_s1.is_pos != node_s2.is_pos)")
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        # print("\n>>>> New invoke: inconsistent_support_mutex")
        #print(node_s1.show())
        #print(node_s2.show())
        #inconsistent_support_mutex = True
        parents1 = node_s1.parents
        parents2 = node_s2.parents
        # TODO test for Inconsistent Support between nodes
        for n_p_1 in parents1:
            for n_p_2 in parents2:
                if not n_p_1.is_mutex(n_p_2) and not n_p_2.is_mutex(n_p_1):
                    #inconsistent_support_mutex = False
                    return False
    ...
    def h_levelsum(self) -> int:
        ...
        :return: int
        """
        # print("\n>>>> New invoke: h_levelsum")
        # TODO implement
        def find_level(self, goal):
            """Find the first level in s_levels where you can find the goal.

            :return: int
            """
            level = 0
            for literals in self.s_levels:
                for literal in literals:
                    # create expresion from literal
                    if literal.is_pos:
                        exp_literal = expr(literal.symbol)
                    else:
                        exp_literal = expr('~{}'.format(literal.symbol))
                    if goal == exp_literal:     
                        return level
                level = level+1
            return False
        ...
        for goal in self.problem.goal:  # Expr object!
            found = find_level(self, goal)
            if found is not False:
                level_sum = level_sum + found
            else: 
                print("\n my_planning_graph.h_levelsum FAIL, Goal not found in s_levels. Return -1")
                return -1
